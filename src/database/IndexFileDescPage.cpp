...
void IndexFileDescPage::print()
{
    ...
    if(getUnique())
    {
        Print::print("This index is a unique one.")
            .print("First bucket page is at ").printLine(getFirstBucketPage());
    }
}
...
{
    int re;
    readInt((*ifds)[FIRST_BUCKET_PAGE_OFFSET], &re);
    return re;
}
...
int IndexFileDescPage::getUnique()
{
    int re;
    readInt((*ifds)[UNIQUE_OFFSET], &re);
    return re == 1;
}
...
{
    writeInt((*ifds)[FIRST_BUCKET_PAGE_OFFSET], n);
}
...
void IndexFileDescPage::setUnique(bool n)
{
    writeInt((*ifds)[UNIQUE_OFFSET], n ? 1 : 0);
}
